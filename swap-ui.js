...
function renderSwapPanel(){
  ...
  if(!window.__swapState)window.__swapState={from:"BNB",to:"USDT",amount:"0.001",slippage:"50",wallet:null,quote:null};
  const s=window.__swapState,from=tokenById(s.from),to=tokenById(s.to);
  root.innerHTML='<div class="swap-card"><div class="swap-head"><div><h3>Live Swap</h3><p class="sub">Non-custodial wallet → PancakeSwap V2 Router → BNB Chain. Quotes and transactions are sent through your wallet provider.</p></div><button type="button" class="swap-connect" id="swapConnect">'+(s.wallet?shortAddress(s.wallet):"Connect Wallet")+'</button></div><div class="live-badge">LIVE · BNB Chain</div><div class="swap-leg"><div class="row"><label>From</label><select id="swapFrom">'+liveTokenOptions(s.from)+'</select></div><input type="number" id="swapAmount" min="0.0001" max="0.005" step="0.0001" value="'+s.amount+'" placeholder="0.001"><div class="swap-meta">Small-balance mode · max 0.005 BNB per swap</div></div><button type="button" class="swap-flip" id="swapFlip">⇅</button><div class="swap-leg"><div class="row"><label>To</label><select id="swapTo">'+liveTokenOptions(s.to)+'</select></div><input type="text" id="swapOut" readonly value="'+(s.quote?s.quote.displayOut:"")+'" placeholder="Live quote · no signing"></div><div class="swap-meta" id="swapMeta">'+(s.quote?("Live quote · 1 "+from.symbol+" ≈ "+s.quote.rate+" "+to.symbol+"<br>Minimum received "+s.quote.minOutDisplay+" "+to.symbol+" · Slippage "+(Number(s.slippage)/100)+"%") :"Connect your wallet, then request a live quote. Quoting uses read-only RPC and never signs a transaction.")+'</div><div class="field"><label>Slippage tolerance</label><select id="swapSlippage" class="expiry"><option value="25" '+(s.slippage==="25"?"selected":"")+'>0.25%</option><option value="50" '+(s.slippage==="50"?"selected":"")+'>0.50%</option><option value="100" '+(s.slippage==="100"?"selected":"")+'>1.00%</option><option value="200" '+(s.slippage==="200"?"selected":"")+'>2.00%</option></select></div><button type="button" class="swap-btn" id="swapQuote" '+(!s.wallet?"disabled":"")+'>Get live quote</button><button type="button" class="swap-btn" id="swapExecute" style="margin-top:8px" '+(!s.wallet||!s.quote?"disabled":"")+'>Swap in wallet</button><p class="swap-note" id="swapResult"></p><p class="swap-note">Live execution is limited to verified crypto contracts. bStocks/RWA remain discovery-only until an exact contract and liquidity route are verified.</p><div class="token-chips">'+allTokenChips()+"</div></div>";
